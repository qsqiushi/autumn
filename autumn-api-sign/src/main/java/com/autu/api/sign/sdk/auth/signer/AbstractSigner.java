...
import com.autu.api.sign.sdk.internal.SdkDigestInputStream;
import com.autu.api.sign.sdk.util.HttpUtils;
import com.autu.api.sign.sdk.util.StringUtils;
import com.autu.api.sign.sdk.Request;
import com.autu.api.sign.sdk.WebServiceRequest;
...
public abstract class AbstractSigner implements Signer {
    ...
    public byte[] hash(String text) throws ClientException {
        try {
            ...
            throw new ClientException("Unable to compute hash while signing request: " + var3.getMessage(), var3);
        }
    }
    ...
    protected byte[] hash(InputStream input) throws ClientException {
        try {
            ...
            throw new ClientException("Unable to compute hash while signing request: " + var5.getMessage(), var5);
        }
    }
    ...
    protected String getCanonicalizedQueryString(Map<String, List<String>> parameters) {
        ...
        while (var11.hasNext()) {
            ...
            String value;
            for (Iterator var13 = ((List)entry.getValue()).iterator(); var13.hasNext();
    ...
    protected byte[] getBinaryRequestPayloadWithoutQueryParams(Request<?> request) {
        ...
        try {
            ...
            throw new ClientException("Unable to read request payload to sign request: " + var15.getMessage(), var15);
        } finally {
            if (byteArrayOutputStream != null) {
                try {
                    byteArrayOutputStream.close();
                } catch (IOException var14) {
                    throw new ClientException("Unable to close byteArrayOutputStream: " + var14.getMessage(), var14);
                }
            }
    ...
    protected InputStream getBinaryRequestPayloadStream(Request<?> request) {
        if (HttpUtils.usePayloadForQueryParameters(request)) {
            String encodedParameters = HttpUtils.encodeParameters(request);
            return encodedParameters == null ? new ByteArrayInputStream(new byte[0])
    ...
    protected InputStream getBinaryRequestPayloadStreamWithoutQueryParams(Request<?> request) {
        try {
            ...
            throw new ClientException("Unable to read request payload to sign request: " + var3.getMessage(), var3);
        }
    }
