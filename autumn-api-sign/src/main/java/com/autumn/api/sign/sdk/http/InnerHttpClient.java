...
public class InnerHttpClient {
  ...
  private static String createUserAgentString(String existingUserAgentString, String userAgent) {
    return existingUserAgentString.contains(userAgent)
        ? existingUserAgentString
  ...
  private <T> void afterResponse(
      Request<?> request, List<RequestHandler> requestHandlers, Response<T> response) {
  ...
  private List<RequestHandler> requestHandler2s(
  ...
  private <T> Response<T> executeOneRequest(
      ...
      InnerHttpClient.ExecOneRequestParams p)
      throws IOException {
  ...
  private <T> T handleResponse(
      HttpResponseHandler<WebServiceResponse<T>> responseHandler, HttpResponse httpResponse)
      throws IOException {
    try {
      WebServiceResponse<? extends T> response =
          (WebServiceResponse) responseHandler.handle(httpResponse);
