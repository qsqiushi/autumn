...
import com.autumn.api.sign.sdk.http.conn.ssl.TLSSocketFactory;
  ...
  public HttpClient createHttpClient(ClientConfiguration config) {
    ...
    if (socketSendBufferSizeHint > 0 || socketReceiveBufferSizeHint > 0) {
      HttpConnectionParams.setSocketBufferSize(
          httpClientParams, Math.max(socketSendBufferSizeHint, socketReceiveBufferSizeHint));
    }
      ...
      if (sf == null) {
        sf =
            new TLSSocketFactory(
                SSLContext.getDefault(), SSLSocketFactory.STRICT_HOSTNAME_VERIFIER);
      }
