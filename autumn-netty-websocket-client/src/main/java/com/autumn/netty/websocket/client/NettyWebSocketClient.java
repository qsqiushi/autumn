...
public class NettyWebSocketClient {
  ...
  public void sendBinaryMessage(byte[] message) {
    this.channelHandlerContext
        .channel()
        .writeAndFlush(new BinaryWebSocketFrame(Unpooled.wrappedBuffer(message)));
  }

  public void sendPingMessage(byte[] message) {
    this.channelHandlerContext
        .channel()
        .writeAndFlush(new PingWebSocketFrame(Unpooled.wrappedBuffer(message)));
  }

  public void sendPongMessage(byte[] message) {
    this.channelHandlerContext
        .channel()
        .writeAndFlush(new PongWebSocketFrame(Unpooled.wrappedBuffer(message)));
  }

  public void sendContinuationMessage(byte[] message) {
    this.channelHandlerContext
        .channel()
        .writeAndFlush(new ContinuationWebSocketFrame(Unpooled.wrappedBuffer(message)));
  }
  ...
  public void heartCheck() {
    ...
    channelHandlerContext
        .pipeline()
        .addLast("heartCheck", new IdleStateHandler(25, 15, 10, TimeUnit.SECONDS));
  }
}
