...
import com.autumn.netty.websocket.server.support.WsPathMatcher;
  ...
  private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
    ...
    if (!req.decoderResult().isSuccess()) {
      if (badRequestByteBuf != null) {
        res =
            new DefaultFullHttpResponse(
                HTTP_1_1, BAD_REQUEST, badRequestByteBuf.retainedDuplicate());
      } else {
    ...
    if (req.method() != GET) {
      if (forbiddenByteBuf != null) {
        res =
            new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN, forbiddenByteBuf.retainedDuplicate());
      } else {
