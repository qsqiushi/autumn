package com.autumn.netty.websocket.server.pojo;

import com.autumn.netty.websocket.server.annotation.BeforeHandshake;
...
public class Session {
  ...
  /**
   * Return the {@link ChannelMetadata} of the {@link Channel} which describe the nature of the
   * {@link Channel}.
   */
  ...
  /**
   * Returns the local address where this channel is bound to. The returned {@link SocketAddress} is
   * supposed to be down-cast into more concrete type such as {@link InetSocketAddress} to retrieve
   ...
   * InetSocketAddress} to retrieve the detailed information.
   *
   * @return the remote address of this channel. {@code null} if this channel is not connected. If
   *     this channel is not connected but it can receive messages from arbitrary remote addresses
   *     (e.g. {@link DatagramChannel}, use {@link DatagramPacket#recipient()} to determine the
   *     origination of the received message as this method will return {@code null}.
   */
  ...
  /**
   * Returns the {@link ChannelFuture} which will be notified when this channel is closed. This
   * method always returns the same future instance.
   */
  ...
  /**
   * Returns {@code true} if and only if the I/O thread will perform the requested write operation
   * immediately. Any write requests made when this method returns {@code false} are queued until
   * the I/O thread is ready to process the queued write requests.
   */
  ...
  /**
   * Get how many bytes can be written until {@link #isWritable()} returns {@code false}. This
   * quantity will always be non-negative. If {@link #isWritable()} is {@code false} then 0.
   */
  ...
  /**
   * Get how many bytes must be drained from underlying buffers until {@link #isWritable()} returns
   * {@code true}. This quantity will always be non-negative. If {@link #isWritable()} is {@code
   * true} then 0.
   */
