...
import com.autumn.netty.websocket.server.standard.ServerEndpointConfig;
...
public class PojoEndpointServer {
  ...
  public static final AttributeKey<Map<String, String>> URI_TEMPLATE =
      AttributeKey.valueOf("WEBSOCKET_URI_TEMPLATE");
  public static final AttributeKey<Map<String, List<String>>> REQUEST_PARAM =
      AttributeKey.valueOf("WEBSOCKET_REQUEST_PARAM");
  ...
  private static final InternalLogger logger =
      InternalLoggerFactory.getInstance(PojoEndpointServer.class);
  ...
  public PojoEndpointServer(
      PojoMethodMapping methodMapping, ServerEndpointConfig config, String path) {
  ...
  public void doOnMessage(Channel channel, WebSocketFrame frame) {
    ...
    if (methodMapping.getOnMessage() != null) {
      ...
      try {
        methodMapping
            .getOnMessage()
            .invoke(implement, methodMapping.getOnMessageArgs(channel, textFrame));
      } catch (Throwable t) {
        logger.error(t);
      }
    }
  ...
  public void doOnBinary(Channel channel, WebSocketFrame frame) {
    ...
    if (methodMapping.getOnBinary() != null) {
      ...
      try {
        methodMapping
            .getOnBinary()
            .invoke(implement, methodMapping.getOnBinaryArgs(channel, binaryWebSocketFrame));
      } catch (Throwable t) {
        logger.error(t);
      }
    }
