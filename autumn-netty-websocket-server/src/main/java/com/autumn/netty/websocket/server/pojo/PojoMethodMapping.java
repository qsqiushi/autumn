...
import com.autumn.netty.websocket.server.annotation.*;
import com.autumn.netty.websocket.server.exception.DeploymentException;
import com.autumn.netty.websocket.server.support.*;
...
public class PojoMethodMapping {
  ...
  public PojoMethodMapping(
      Class<?> pojoClazz, ApplicationContext context, AbstractBeanFactory beanFactory)
      ...
      for (Method method : currentClazzMethods) {
        if (method.getAnnotation(BeforeHandshake.class) != null) {
          ...
          if (close == null) {
            ...
            if (currentClazz == pojoClazz || !isMethodOverride(close, method)) {
              // Duplicate annotation
          ...
          if (error == null) {
            ...
            if (currentClazz == pojoClazz || !isMethodOverride(error, method)) {
              // Duplicate annotation
          ...
          if (message == null) {
            ...
            if (currentClazz == pojoClazz || !isMethodOverride(message, method)) {
              // Duplicate annotation
          ...
          if (binary == null) {
            ...
            if (currentClazz == pojoClazz || !isMethodOverride(binary, method)) {
              // Duplicate annotation
          ...
          if (event == null) {
            ...
            if (currentClazz == pojoClazz || !isMethodOverride(event, method)) {
              // Duplicate annotation
  ...
  private boolean isOverridenWithoutAnnotation(
      Method[] methods, Method superclazzMethod, Class<? extends Annotation> annotation) {
  ...
  Object[] getBeforeHandshakeArgs(Channel channel, FullHttpRequest req) throws Exception {
    return getMethodArgumentValues(
        channel, req, beforeHandshakeParameters, beforeHandshakeArgResolvers);
  }
  ...
  Object[] getOnMessageArgs(Channel channel, TextWebSocketFrame textWebSocketFrame)
      throws Exception {
    return getMethodArgumentValues(
        channel, textWebSocketFrame, onMessageParameters, onMessageArgResolvers);
  }
  ...
  Object[] getOnBinaryArgs(Channel channel, BinaryWebSocketFrame binaryWebSocketFrame)
      throws Exception {
    return getMethodArgumentValues(
        channel, binaryWebSocketFrame, onBinaryParameters, onBinaryArgResolvers);
  }
  ...
  private Object[] getMethodArgumentValues(
      ...
      MethodArgumentResolver[] resolvers)
      throws Exception {
  ...
  private MethodArgumentResolver[] getResolvers(MethodParameter[] parameters)
      throws DeploymentException {
    MethodArgumentResolver[] methodArgumentResolvers =
        new MethodArgumentResolver[parameters.length];
    ...
    for (int i = 0; i < parameters.length; i++) {
      ...
      if (methodArgumentResolvers[i] == null) {
        throw new DeploymentException(
            "pojoMethodMapping.paramClassIncorrect parameter name : "
