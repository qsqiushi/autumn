...
public class GrayReactiveLoadBalancerClientFilter implements GlobalFilter, Ordered {
  ...
  @Override
  public int getOrder() {
    // 这里要注意loadBalanceClientFilter的order是10100,这里要比他小一点？？？？？
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    String schemePrefix =
        (String) exchange.getAttribute(ServerWebExchangeUtils.GATEWAY_SCHEME_PREFIX_ATTR);
              ...
              (response) -> {
                if (!response.hasServer()) {
                  throw NotFoundException.create(
                      this.properties.isUse404(), "Unable to find instance for " + url.getHost());
                } else {
  ...
  private Mono<Response<ServiceInstance>> choose(ServerWebExchange exchange) {
    ...
    GrayLoadBalancer loadBalancer =
        new GrayLoadBalancer(
            ...
            uri.getHost(),
            new Strategy().setLoadBalancerEnum(LoadBalancerEnum.VERSION));
