...
public class DynamicRouteServiceImpl implements ApplicationEventPublisherAware, IDynamicRouteService {
    ...
    @Resource
    private ResourceConfigParamMapper resourceConfigParamMapper;
    ...
    public void routeInit() {
        ...
        List<com.autumn.gateway.entity.Resource> resourceList = resourceMapper.selectList(
                new QueryWrapper<com.autumn.gateway.entity.Resource>().lambda()
        ...
        for (com.autumn.gateway.entity.Resource resource : resourceList) {
            ...
            if (resource.getGroupId() != null) {
                ...
                headerPredicate.setName(NameUtils.normalizeRoutePredicateName(HeaderRoutePredicateFactory.class));
            ...
            pathPredicateDefinition.setName(NameUtils.normalizeRoutePredicateName(PathRoutePredicateFactory.class));
            ...
            if (resource.getUrl().contains(",")) {
                ...
                for (int i = 0; i < urls.length; i++) {
                    ...
                    if (urls[i].endsWith("**")) {
                        api.getPredicateItems().add(new ApiPathPredicateItem().setPattern(urls[i]).setMatchStrategy(SentinelGatewayConstants.URL_MATCH_STRATEGY_PREFIX));
                    } else {
                ...
                pathPredicateDefinition.getArgs().put(PathRoutePredicateFactory.PATTERN_KEY, resource.getUrl());

                if (resource.getUrl().endsWith("**")) {
                    api.getPredicateItems().add(new ApiPathPredicateItem().setPattern(resource.getUrl()).setMatchStrategy(SentinelGatewayConstants.URL_MATCH_STRATEGY_PREFIX));
                } else {
            ...
            List<ResourceConfigInfo> resourceConfigInfoList = resourceConfigMapper.selectResourceConfigInfo(
                    new QueryWrapper<ResourceConfig>().lambda().eq(ResourceConfig::getResourceId, resource.getId())
            ...
            for (ResourceConfigInfo resourceConfigInfo : resourceConfigInfoList) {
                ...
                List<ResourceConfigParamPO> resourceConfigParamList = resourceConfigParamMapper.selectResourceConfigParamInfo(new QueryWrapper<ResourceConfigParam>()
                        .lambda().eq(ResourceConfigParam::getResourceConfigId, resourceConfigInfo.getId())
                ...
                switch (resourceConfigInfo.getConfig().getType()) {
                    case FILTER:
                        ...
                        for (ResourceConfigParamPO resourceConfigParamPO : resourceConfigParamList) {
                            filterDefinition.getArgs().put(resourceConfigParamPO.getConfigParam().getCode(), resourceConfigParamPO.getValue());
                        }
                    ...
                    case PREDICATE:
                        ...
                        for (ResourceConfigParamPO resourceConfigParamPO : resourceConfigParamList) {
                            predicateDefinition.getArgs().put(resourceConfigParamPO.getConfigParam().getCode(), resourceConfigParamPO.getValue());
                        }
                        routeDefinition.getPredicates().add(pathPredicateDefinition);
                        break;
                    case GATEWAY_FLOW_RULE:

                        Map<String, String> map = resourceConfigParamList.stream().collect(Collectors.toMap(e -> e.getConfigParam().getCode(), e -> e.getValue()));

                        GatewayFlowRule gatewayFlowRule = new GatewayFlowRule(resource.getId())
                                // 限流的阈值  2线程或者2qps
                                .setCount(map.get("count") == null ? 10 : Integer.parseInt(map.get("count")))
                                // 限流阈值类型，qps 或线程数
                                .setGrade(map.get("grade") == null ? RuleConstant.FLOW_GRADE_QPS : Integer.parseInt(map.get("grade")))
                                //intervalSec：统计时间窗口，单位是秒，默认是 1 秒 目前仅对参数限流生效。
                                .setIntervalSec(map.get("intervalSec") == null ? 1 : Integer.parseInt(map.get("intervalSec")))
                                //应对突发请求时额外允许的请求数目
                                ...
                                .setControlBehavior(map.get("controlBehavior") == null ? RuleConstant.CONTROL_BEHAVIOR_RATE_LIMITER : Integer.parseInt(map.get("controlBehavior")))
                                //匀速排队模式下的最长排队时间，单位是毫秒，仅在匀速排队模式下生效。
                                .setMaxQueueingTimeoutMs(map.get("maxQueueingTimeoutMs") == null ? 600 : Integer.parseInt(map.get("maxQueueingTimeoutMs")));
                        ...
                        if (map.get("paramItem") != null) {
                            ...
                            switch (Integer.parseInt(map.get("paramItem"))) {
                                case SentinelGatewayConstants.PARAM_PARSE_STRATEGY_CLIENT_IP:
                                    gatewayFlowRule.setParamItem(new GatewayParamFlowItem().setParseStrategy(SentinelGatewayConstants.PARAM_PARSE_STRATEGY_CLIENT_IP));
                                    break;
                                case SentinelGatewayConstants.PARAM_PARSE_STRATEGY_HOST:
                                    gatewayFlowRule.setParamItem(new GatewayParamFlowItem().setParseStrategy(SentinelGatewayConstants.PARAM_PARSE_STRATEGY_HOST));
                                    break;
                                ...
                                case SentinelGatewayConstants.PARAM_PARSE_STRATEGY_HEADER:
                                    ...
                                    gatewayFlowRule.setParamItem(new GatewayParamFlowItem().setParseStrategy(SentinelGatewayConstants.PARAM_PARSE_STRATEGY_HEADER)
                                            .setFieldName(map.get("fieldName")));
                                    break;

                                case SentinelGatewayConstants.PARAM_PARSE_STRATEGY_URL_PARAM:
                                    gatewayFlowRule.setParamItem(new GatewayParamFlowItem().setParseStrategy(SentinelGatewayConstants.PARAM_PARSE_STRATEGY_URL_PARAM)
                                            .setFieldName(map.get("fieldName")));
                                    break;
                                default:
                                    break;
                        ...
                        break;
                    case DEGRADE_RULE:
                        Map<String, String> degradeRuleParamMap = resourceConfigParamList.stream().collect(Collectors.toMap(e -> e.getConfigParam().getCode(), e -> e.getValue()));

                        DegradeRule degradeRule = new DegradeRule();
                        //资源名
                        degradeRule.setResource(resource.getId());
                        //平均响应时间阈值 慢调用比例模式下为慢调用临界 RT（超出该值计为慢调用）；异常比例/异常数模式下为对应的阈值
                        degradeRule.setCount(degradeRuleParamMap.get("count") == null ? 500 : Integer.parseInt(degradeRuleParamMap.get("count")));
                        ...
                        degradeRule.setGrade(degradeRuleParamMap.get("grade") == null ? RuleConstant.DEGRADE_GRADE_RT : Integer.parseInt(degradeRuleParamMap.get("grade")));
                        //设置  降级时间 熔断时长，单位为 s
                        degradeRule.setTimeWindow(degradeRuleParamMap.get("timeWindow") == null ? 10 : Integer.parseInt(degradeRuleParamMap.get("timeWindow")));
                        //熔断触发的最小请求数，请求数小于该值时即使异常比率超出阈值也不会熔断（1.7.0 引入）
                        degradeRule.setMinRequestAmount(degradeRuleParamMap.get("minRequestAmount") == null ? 5 : Integer.parseInt(degradeRuleParamMap.get("minRequestAmount")));
                        //统计时长（单位为 ms），如 60*1000 代表分钟级（1.8.0 引入）
                        degradeRule.setStatIntervalMs(degradeRuleParamMap.get("statIntervalMs") == null ? 1000 : Integer.parseInt(degradeRuleParamMap.get("statIntervalMs")));
                        //慢调用比例阈值，仅慢调用比例模式有效（1.8.0 引入）
                        degradeRule.setSlowRatioThreshold(degradeRuleParamMap.get("slowRatioThreshold") == null ? 0.5 : Double.parseDouble(degradeRuleParamMap.get("slowRatioThreshold")));
                    ...
                    case AUTHORITY_RULE:
                        ...
                        Map<String, String> authorityRulesRuleParamMap = resourceConfigParamList.stream().collect(Collectors.toMap(e -> e.getConfigParam().getCode(), e -> e.getValue()));

                        if (authorityRulesRuleParamMap.get("limitApp") != null) {
                            AuthorityRule authorityRule = new AuthorityRule();
                            authorityRule.setResource(resource.getId());
                            authorityRule.setStrategy(authorityRulesRuleParamMap.get("strategy") == null ? RuleConstant.AUTHORITY_WHITE : Integer.parseInt(authorityRulesRuleParamMap.get("strategy")));
        ...
        //指定TokenServer的Ip和地址
        ClusterClientAssignConfig clusterClientAssignConfig=new ClusterClientAssignConfig("127.0.0.1",18888);
        //应用
    ...
    public void hystrix() {
        FilterDefinition filterDefinition = new FilterDefinition();
        filterDefinition.setName(NameUtils.normalizeFilterFactoryName(HystrixGatewayFilterFactory.class));
        filterDefinition.setArgs(new HashMap<>());
    ...
    public void retry() {
        ...
        retryFilterDefinition.setName(NameUtils.normalizeFilterFactoryName(RetryGatewayFilterFactory.class));
